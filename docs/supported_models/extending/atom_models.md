...
ATOM (AiTer Optimized Model) is a high-performance model implementation backend specifically optimized for AMD Instinct GPUs. This doc guides users to run ATOM-accelerated models in SGLang, delivering optimal performance on AMD Instinct GPU devices (e.g., MI300 series).

## Requirements

Users need to install ATOM. ATOM only runs on AMD ROCm GPUs.
...
```bash
# Install the AITER kernel library
pip install amd-aiter

# Clone and install the ATOM repository
...
- **Optimized Kernels**: Custom-tuned AITER kernels (ASM, CK, Triton) for AMD GPUs
